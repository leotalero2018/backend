        ...
        def validate_environment(cls, values: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                if values.get("streaming", True):
                    values["client"] = GrpcTritonClient(values["server_url"], ssl=False)
                else:
...
class GrpcTritonClient(_BaseTritonClient):
    ...
    def __init__(self, url: str, ssl: bool = True):
        """Initialize the GRPC client with SSL option."""
        self._client = grpcclient.InferenceServerClient(url=url, ssl=ssl)
    ...
    def request_streaming(
        ...
        **params: Any,
        ...
        print("----request_streaming-----model_name:", model_name)
        if not self._client.is_model_ready(model_name):
